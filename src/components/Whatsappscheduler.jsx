...
import { whatsappGetTemplateById, whatsappSchedulerTemplate, whatsappSchedulerTemplateById, WhatsappTemplateType } from '../api/Api';
import useMyContext from '../usercontext/useMyContext';

export default function EnhancedWhatsAppScheduler() {
  ...
  useEffect(() => {
    const fetchExcelType = async () => {
      try {
        const response = await whatsappSchedulerTemplate();
  ...
  useEffect(() => {
    const fetchClients = async () => {
      ...
      try {
        const response = await whatsappSchedulerTemplateById(excelType.id, { page: currentPage });
        ...
        <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          <Box sx={{ flex: 1, minWidth: 0 }}>
            ...
            <Chip 
              ...
              size="small"
              color={
                client.status === 'VIP' ? 'warning' :
                client.status === 'Active' ? 'success' : 'default'
              }
              sx={{
                bgcolor: client.status === 'VIP' ? themeColors.warning :
                         client.status === 'Active' ? themeColors.success : themeColors.disabled,
                color: 'white',
                fontSize: '0.75rem'
              }}
            />
          ...
          renderOption={(props, option) => (
            <Box component="li" {...props}>
              <Box>
                ...
                <Typography variant="caption" sx={{ color: themeColors.textSecondary }}>{option.description}</Typography>
              </Box>
            </Box>
          )}
          sx={{ mb: 3 }}
                    ...
                    <Table size="small">
                      <TableHead>
                        <TableRow sx={{ bgcolor: themeColors.tableHeaderBg }}>
                          ...
                          <TableCell sx={{ color: themeColors.textPrimary }}><strong>Status</strong></TableCell>
                        </TableRow>
                      </TableHead>
                      <TableBody>
